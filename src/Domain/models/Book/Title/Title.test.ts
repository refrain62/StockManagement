...
describe('Title', () => {
  ...
  test('最小以上の値でTitleを生成するとエラーを投げる', () => {
    expect(() => new Title('')).toThrow('タイトルは1文字以上、1000文字以下でなければなりません。')
  })

  test('最大長以上の値でtitleを生成するとエラーを投げる', () => {
    const tooLongTitle = 'a'.repeat(1001)

    expect(() => new Title(tooLongTitle)).toThrow('タイトルは1文字以上、1000文字以下でなければなりません。')
  })
})
